...
<%
    ...
    String enrollDate = request.getAttribute("enrollDate") == null ? "" : (String)request.getAttribute("enrollDate");
	                ...
	                <form id="search1" method="post" action="">
	                	<div id="myDona">
							...
							<div id="donaTable">
								<div class="admin-donation-search" style="z-index:100 !important;">
									<div class="form-group row" style="margin-left: 380px; margin-top: -20px;">
										<div style="margin-right:10px;">
											...
											<select id="selectBox" class="selectpicker show-tick" data-style="btn-primary" data-width="130px" onchange="OnChange();">
												...
												<option value="optionEnrollDate" selected>가입일</option>
												<option value="optionName" >이름</option>
											</select>
										</div>	                                
									<!-- 검색내용 입력 칸 -->
									<div class="input-group" style="width: 250px;">
										<input type="text" id="search2" class="form-control" name="" placeholder="내용을 입력해주세요" value="<%= enrollDate %>">
									...
									<div style="margin-left:150px;">
										<button type="button" onclick="window.location.href='memberListAll.me?currentPage=1'" class="btn btn-primary">전체목록으로</button>
									</div>
								</div>
							</div> 
							...
							function OnChange() {
								...
								if(gubun == "optionNo"){
									search1.action = "<%= contextPath %>/memberListMemNo.me?currentPage=1";
									search2.name = "memNo";
								} 
								else if(gubun == "optionEnrollDate"){
									search1.action = "<%= contextPath %>/memberListDate.me?currentPage=1";
									search2.name = "enrollDate";
								} 
								else if(gubun == "optionName"){
									search1.action = "<%= contextPath %>/memberListMemName.me?currentPage=1";
							...
							<nav aria-label="Page navigation example">
								<ul class="pagination" style="justify-content: center;"> <!-- justify-content: center; : 페이징 가운데 정렬-->
									<li class="page-item">
										<% if(currentPage != 1){ %>
											<a class="page-link" href="<%= contextPath %>/memberListDate.me?currentPage=<%= currentPage - 1 %>" aria-label="Previous">
									...
									<% if(p != currentPage){ %>
									<li class="page-item"><a class="page-link" href="<%= contextPath %>/memberListDate.me?currentPage=<%= p %>"><%= p %></a></li>
										<% } else{ %>
									...
									<li class="page-item">  
										<% if(currentPage != maxPage){%>
											<a class="page-link" href="<%= contextPath %>/memberListDate.me?currentPage=<%= currentPage + 1 %>" aria-label="Next">
