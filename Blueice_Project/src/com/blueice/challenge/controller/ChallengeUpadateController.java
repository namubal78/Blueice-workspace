...
/**
 * Servlet implementation class ChallengeUpadateController
 */
@WebServlet("/update.challenge") // 챌린지 게시글 수정을 위한 서블릿
public class ChallengeUpadateController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChallengeUpadateController() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			c.setChalNo(Integer.parseInt(multiRequest.getParameter("cno")));
			c.setChalTitle(multiRequest.getParameter("title"));
			c.setChalConts(multiRequest.getParameter("content"));
			c.setChallengeOriginName(multiRequest.getOriginalFileName("reUpfile"));
			c.setChallengeChangeName(multiRequest.getFilesystemName("reUpfile"));
			c.setChallengeFilePath("resources/images/event/");
			
			int result = new ChallengeService().updateChallenge(c);
