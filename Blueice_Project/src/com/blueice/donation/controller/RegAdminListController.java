 ...
 */
@WebServlet("/regAdminlist.do")
public class RegAdminListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RegAdminListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// DonationService 자주 쓸 거니까 변수로 만들어놓기
		DonationService ds = new DonationService();
		
		// 검색어를 담을 변수 세팅
		String searchText = null;
		
		// 검색어가 null이 아니면 (검색어가 있으면) searchText 변수에 담기
		...
		if (searchText == null && selection == null) {
			// 기본 전체 리스트 조회를 위한 listCount 구하기
			listCount = ds.selectListCount(type);
		} else {
			// 검색어 포함 전체 리스트 조회를 위한 listCount 구하기
			switch (selection) {
			...
			case "type" : 	
				...
				if(memType.equals("personal")) { 
					memType = "1";
				} else if(memType.equals("group")) { 
					memType = "2";
				} else { 
					memType = "3"; 
				}
		...
		// VO를 담을 리스트
		ArrayList<DonationRegular> list = null;
		ArrayList<Member> list2 = new ArrayList<>();
		
		
		if (searchText == null && selection == null) { // 검색어가 없으면
			// DonationRegular 리스트 service 단에서 불러오기
			list = ds.selectRegDonaList(pi);
			
		} else { // selection에 따른 검색어가 있으면
			switch (selection) {
			case "name" : list = ds.selectRegSearchNameList(pi, searchText); break;
			case "memId" : list = ds.selectRegSearchIdList(pi, searchText); break;
			case "type" : list = ds.selectRegSearchTypeList(pi, memType); break;
			}
		}
		
		// 각 결제건에 대한 멤버 객체 list 받아오기
